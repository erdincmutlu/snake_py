...
while run:
    ...
    if snake_x == food_x and snake_y == food_y:
        ...
        while food_on_snake:
            food_x = (
                round(
                    random.randrange(0, window_width - food_block_size)
                    / snake_block_size
                )
                * snake_block_size
            )
            food_y = (
                round(
                    random.randrange(0, window_height - food_block_size)
                    / snake_block_size
                )
                * snake_block_size
            )
    ...
    if keys[pygame.K_LEFT] and snake_x_change != snake_block_size:
        ...
        snake_x_change = 0
    elif keys[pygame.K_DOWN] and snake_x_change != -snake_block_size:
        snake_y_change = snake_block_size
        snake_x_change = 0
